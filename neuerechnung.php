...
ob_start();
	 ...
	 if(isset($_REQUEST['invid']) && $_REQUEST['invid'] != "")
	 {
          ...
          $in_sql = "SELECT invc.*, usr.vFname, usr.vLname, usr.vCustomerNumber, tax.vVat FROM invoice_mst as invc
     ...
     if (isset($_REQUEST['send'])) // Save data
	 {
		  ...
		  $invoice_number = mysql_real_escape_string($_REQUEST['invoice_number']);
		   ...
		   ////////////
		$sql = "SELECT * FROM invoice_mst where vInvoiceNumber = '".$invoice_number."' ";
		...
		else
		{
          ...
          $addedby = $_SESSION['uid'];
			...
			if($isUpdate)
			{
			 $invoice_add = "UPDATE invoice_mst SET `iCustomerId` = '".$customer_id."', `vInvoiceNumber` = '".$invoice_number."', `dInvoiceDate` = '".$invoice_date."', `iTermOfPayment` = '".$payment_term."', `iPayment` = '".$payment_type."', `vShipping` = '".$shipping."', `iTex` = '".$tax_id."', `tComment` = '".$comment."', `iAddedBy` = '".$addedby."', `dAddedDate` = '".$date."' where `iId` =  '".$nextid."' ";
			}
		  else
		  {
		...
		$tem_dynamic = $invoice_row['dynamic'];
		  ...
		  $sub_tot = 0;
           ...
           mysql_query("DELETE FROM invoice_items where `iInvoiceId` = '".$nextid."' ");
          ...
          foreach($_REQUEST['qty'] as $key => $value)
		  {
               ...
               $date = date('Y-m-d H:i:s');
               $item_sql = "INSERT INTO invoice_items (`iInvoiceId`, `iItemOrder`, `vQTY`, `tDescription`, `vPrice`, `iAddedBy`, `dAddedDate`) VALUES ('".$nextid."', '".($key+1)."', '".$qty."', '".$description."', '".$price."', '".$addedby."', '".$date."')";
...
<li><a href="geschlossenetickets.php"><span class="icon"><img src="img/icons/packs/fugue/16x16/mail.png" alt="" height=16 width=16></span>Geschlossene Tickets</a></li>
    ...
    <span class="invoice_item" id="item_1">
       ...
       <p class="_20">
			...
			$invoice_items = "SELECT * FROM invoice_items WHERE iInvoiceId='".$invid."';";
