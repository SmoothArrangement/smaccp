                            ...
                            <table class="dynamic styled" data-filter-Bar="always">
                                ...
                                <tbody>
                                    ...
                                    while ($row = mysql_fetch_assoc($invoice)) {
                                        $invoice_items = "SELECT * FROM offer_items WHERE iInvoiceId='" . $row['iId'] . "';";
...
?>																
        ...
        <script>
            $$.loaded();
            $(".ord_status").live('change', function (e) {
                var status = $(this).val();
                var inv_id = $(this).attr('data-id');
                $.ajax({
                    method: "POST",
                    url: "ajax_save_offer.php",
                    data: {type: 'ord_status', status: status, inv_id: inv_id}
                }).done(function (data) {
                    alert(data);
                });
            });
